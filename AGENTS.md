...
> **Status 2026-09-15 (rev.10):** Merge PR #18 (organizer-refund-payout) + #19 (refund) ke `main`. Tambah **LegalController** (`/terms-conditions`, `/privacy-policy`), **OrganizerController** (`/organizer/**`, 20 endpoint — MOCK/stub), **RefundController** (`/api/refund/**` + `/api/tickets/refund/**`, 6 endpoint, submit REAL), **MidtransService** (real Snap gateway). `PaymentController` kini return `snapToken`/`redirectUrl` via Midtrans, BUKAN mock VA. Entity baru `RefundRequestEntity`, repo `RefundRepository`, DTO `BankResponse`/`RefundDetailResponse`/`RefundRequest`. ⚠️ `AdminSettingsController` + `AdminEoController` ADA DI BRANCH `feat/admin-eo-audit` (commit `a3c95e5`) tapi TIDAK ADA DI `main` — auto-stash menghapusnya dari working tree.
...
│   ├── PaymentController.java        # GET /api/v1/payments/methods + POST /api/v1/payments/charge (auth) → Midtrans Snap (snapToken/redirectUrl), bukan mock VA lagi
...
│   ├── LegalController.java          # GET /terms-conditions, /privacy-policy → static content (PUBLIC)
│   ├── OrganizerController.java      # /organizer/** (20 endpoint — MOCK/stub, perlu login)
│   ├── RefundController.java         # /api/tickets/refund/request, /api/refund/banks|/order-summary|/refund-detail/info|/refund-detail/download-proof|/refund/refund-history (perlu login, submit REAL)
│   ├── admin/
│   │   ├── AdminDashboardController.java # GET /admin/dashboard/metrics|recent-events|recent-transactions (ADMIN only, rev.9)
│   │   └── AdminUserController.java      # GET /admin/users|/{id}, PATCH /{id}/status|/suspend (ADMIN only, rev.9)
│   │   └── # AdminSettingsController + AdminEoController ADA DI BRANCH feat/admin-eo-audit (commit a3c95e5), TIDAK ADA DI main
...
│   └── SecurityConfig.java           # BCrypt, STATELESS, permitAll: /,/error + /api/v1/auth/** + /api/v1/home/** + /api/v1/search/** + /api/v1/events/** + /api/v1/terms-conditions + /api/v1/privacy-policy; `/admin/**` → `hasRole("ADMIN")` (rev.9); 401/403 via ObjectMapper
├── security/
...
│   ├── MidtransService.java          # createSnapTransaction(orderId, grossAmount, customerName, customerEmail) → RestTemplate ke Midtrans Snap API sandbox (application.properties:64-66)
│   ├── LegalService.java             # getTermsAndConditions() + getPrivacyPolicy() → static Map (public endpoints)
│   ├── OrganizerService.java         # /organizer/** 20 endpoint — SEMUA MOCK/stub Map (register, documents, profile, dashboard, auth, refunds, payouts)
│   ├── RefundService.java            # getSupportedBanks() + getRefundOrderSummary() + submitRefund(REAL→DB) + getRefundDetail(REAL→DB) + getRefundHistory(MOCK)
...
│   ├── PaymentChargeRequest.java     # orderId, grossAmount, customerName, customerEmail — digunakan Midtrans Snap (VERIFY: charge sekarang pakai Map langsung)
│   ├── PaymentChargeResponse.java    # snapToken, redirectUrl — response Midtrans Snap (VERIFY: lama punya virtualAccountNumber, sudah tidak akurat)
...
│   ├── BankResponse.java             # bankCode, bankName — daftar bank pendukung refund
│   ├── RefundDetailResponse.java     # refundId, orderId, amount, reason, bankName, accountNumber, accountHolderName, status (PENDING/APPROVED/REJECTED/COMPLETED), proofUrl, createdAt
│   └── RefundRequest.java            # orderId, reason, bankCode, accountNumber, accountHolderName — DTO request ajukan refund
...
│   ├── RefundRequestEntity.java      # refund_requests — @ManyToOne Customer+Order, orderId UUID, customerId UUID, refundAmount, reason, bankName, bankAccountNumber, bankAccountName, status, createdAt
...
│   ├── RefundRepository.java         # extends JpaRepository<RefundRequestEntity, UUID>
│   └── AuditLogRepository.java       # log only
└── service/
    ├── AuthService.java              # register + login + loginWithGoogle + verifyOtp + resendOtp + resetPassword (langsung di auth.resetToken) + logout

**DIHAPUS & JANGAN DIBUAT ULANG:** `RescheduleRequest` + `PasswordResetToken`/`password_reset_tokens` (sudah digabung ke `auth` sesuai mentor). PR #9 mengisi 7 placeholder PR #8; PR #10 lanjut isi `attendees` real + `calculation`/`process`/`orders/status`; PR #11 isi User/Profile + logout; PR #12 isi Ticket (issued-detail + my-tickets alias). PR #18+#19 tambah Organizer, Refund, Legal, Midtrans — sisa yang belum ada (jangan buat kecuali diminta): service/controller/DTO untuk Settings, Reschedule.
...
### ✅ AKTIF — Legal — Syarat & Privasi (Public)
| Method | Endpoint | Auth | Flow |
|---|---|---|---|
| GET | `/terms-conditions` | Public | `LegalService.getTermsAndConditions()` → `{title:"Syarat dan Ketentuan EventDay", content, updated_at:"2026-09-14"}` |
| GET | `/privacy-policy` | Public | `LegalService.getPrivacyPolicy()` → `{title:"Kebijakan Privasi EventDay", content, updated_at:"2026-09-14"}` |

> ⚠️ **Path mismatch**: Controller mapping di root (`/terms-conditions`), bukan `/api/v1/terms-conditions`. `SecurityConfig` permit `/api/v1/terms-conditions` + `/api/v1/privacy-policy` — endpoint sebenarnya `/terms-conditions` (tanpa `/api/v1`). `/api/v1/terms-conditions` → **404** (controller tidak map di sana).

### ✅ AKTIF — Organizer / EO Module (perlu login — `OrganizerController`, `/organizer/**`)
> **Status: MOCK/STUB** — semua return data statis `Map`, **belum persisten ke DB**. Gunakan untuk integrasi UI saja.
| Method | Endpoint | Deskripsi |
|---|---|---|
| POST | `/organizer/register` | Daftar EO → `{organizer_name, verification_status: "PENDING"}` |
| POST | `/organizer/documents/upload?type=` | Upload dokumen (multipart) → `{document_type, file_name, file_size}` |
| GET | `/organizer/status` | Status verifikasi EO (mock `PENDING`) |
| GET | `/organizer/dashboard` | Metrics dashboard (mock: `total_revenue, active_events, tickets_sold`) |
...
| POST | `/organizer/profile/upload-deed` | Upload akta perusahaan (mock URL) |
...
### ✅ AKTIF — Refund Customer (perlu login — `RefundController`, `/api/**`)
| Method | Endpoint | Auth | Flow |
|---|---|---|---|
| POST | `/api/tickets/refund/request` (alias `/api/refund/submit`) | Bearer | `RefundRequest{orderId, reason, bankCode, accountNumber, accountHolderName}` → `RefundService.submitRefund` simpan ke `refund_requests` (REAL). `customerId` dari JWT, `refundAmount` di-hardcode `290000.00` → return `RefundDetailResponse` |
| GET | `/api/refund/banks` | Bearer | `List<BankResponse>{bankCode, bankName}` → BCA/MANDIRI/BNI/BRI |
| GET | `/api/refund/order-summary?orderId=<uuid>` | Bearer | Mock: `{orderId, ticketQuantity:2, grossAmount:300000, adminFee:10000, refundableAmount:290000}` |
| GET | `/api/refund/refund-detail/info?refundId=<uuid>` | Bearer | `RefundService.getRefundDetail` → `RefundDetailResponse` dari DB (REAL) |
| GET | `/api/refund/refund-detail/download-proof?refundId=<uuid>` | Bearer | → `{proofUrl}` |
| GET | `/api/tickets/refund/refund-history?email=<email>` | Bearer | `RefundService.getRefundHistory` → mock 1 item (`COMPLETED`) |

### ⏳ SCHEMA-ONLY (jangan implement kecuali diminta)
`GET/PUT /api/settings`, `POST /api/v1/refunds` (versi lama, sudah diganti `/api/tickets/refund/request`). **Organizer** dan **Legal** SUDAH AKTIF (bukan schema-only lagi). `AdminSettingsController` + `AdminEoController` ada di branch `feat/admin-eo-audit` (commit `a3c95e5`) tapi **TIDAK ADA DI main**. (User profile/transactions/avatar/logout **SUDAH AKTIF** via PR #11.) `/api/v1/terms-conditions` + `/api/v1/privacy-policy` permitAll tapi **mismatch path** — controller map di `/terms-conditions` bukan `/api/v1/...`.
...
- Payment PR #9+rev.10: **Midtrans Snap REAL gateway** (bukan mock VA lagi). `PaymentController` → `MidtransService.createSnapTransaction()` → return `{snapToken, redirectUrl}`. Config `midtrans.server-key`, `midtrans.snap-url` di `application.properties`. Webhook `/api/payments/midtrans-notification` (log only). `PaymentChargeRequest.java` / `PaymentChargeResponse.java` mungkin perlu verify ulang — response sekarang `snapToken`/`redirectUrl`, bukan `virtualAccountNumber`.
...
12. **Status rev.10 (15 Sep)**: Merge PR #18 (#19) ke `main`. `PaymentController` kini pakai **Midtrans Snap REAL** (bukan mock VA). Tambah Legal (2 endpoint), Organizer (20 endpoint mock), Refund (6 endpoint, submit REAL). `RefundRequestEntity` + `RefundRepository` baru. ⚠️ `AdminSettingsController` + `AdminEoController` ADA DI BRANCH `feat/admin-eo-audit` (commit `a3c95e5`) TAPI **TIDAK ADA DI main** — auto-stash menghapusnya dari working tree.
13. **`PaymentChargeRequest.java` / `PaymentChargeResponse.java`**: DTO mungkin sudah tidak akurat untuk Midtrans flow — `PaymentChargeResponse` masih punya `virtualAccountNumber` tapi response sekarang `snapToken`/`redirectUrl`. Verify ulang.
...
> **Terakhir diperbarui:** 15 September 2026. Lihat `API.md` section "GAP ANALYSIS" untuk detail lengkap.

### Ringkasan Cepat

| Modul | Total | ✅ REAL | ⚠️ MOCK | ❌ BELUM | Keterangan |
|---|---|---|---|---|---|
| 01. Home & Search | 6 | 6 | 0 | 0 | **SELESAI** |
| 02. Events & Detail | 8 | 7 | 0 | 1 | Missing: `POST /events/create` |
| 03. Checkout & Payment | 11 | 10 | 1 | 0 | Payment = Midtrans Snap REAL; webhook log only |
| 04. My Tickets | 4 | 4 | 0 | 0 | **SELESAI** (tapi `generateTicket()` tak dipanggil) |
| 05. Refund | 7 | 4 | 3 | 0 | Submit REAL; order-summary/history MOCK |
| 06. User Profile | 6 | 5 | 1 | 0 | Avatar masih mock |
| 07. Organizer & Legal | 6 | 2 | 4 | 0 | Legal REAL; Organizer semua MOCK |
| **Admin Dashboard** | 3 | 3 | 0 | 0 | **SELESAI** |
| **Admin Users** | 4 | 4 | 0 | 0 | **SELESAI** |
| Admin EO Apps | 4 | 0 | 0 | 4 | ❌ BELUM ADA |
| Admin Payouts | 4 | 0 | 0 | 4 | ❌ BELUM ADA |
| Admin Audit/Settings | 4 | 0 | 0 | 4 | ❌ BELUM ADA |
| Organizer Events | 6 | 0 | 0 | 6 | ❌ BELUM ADA |
| Organizer Dashboard | 3 | 0 | 0 | 3 | ❌ BELUM ADA |
| **TOTAL** | **76** | **45** | **9** | **22** | |

### Services yang Sudah Ada (15 service)

| Service | Status | Keterangan |
|---|---|---|
| `AuthService` | ✅ REAL | Register, login, Google, OTP, reset, logout |
| `EventService` | ✅ REAL | Catalog, featured, detail |
| `HomeSearchService` | ✅ REAL | Hero, cards, search, locations, categories |
| `CheckoutService` (via `OrderService`) | ✅ REAL | Initiate, attendees, calculation, process |
| `PaymentService` | ✅ REAL | Summary + charge (Midtrans Snap) |
| `MidtransService` | ✅ REAL | Snap gateway integration |
| `TicketService` | ✅ REAL | My-tickets, issued-detail, scan (tapi `generateTicket` tak dipanggil) |
| `UserService` | ✅ REAL | Profile, update, change-password, history |
| `EmailService` | ✅ REAL | OTP, reset, password-changed, order-confirm |
| `AuditLogService` | ✅ REAL | Log only |
| `LegalService` | ✅ REAL | Terms & privacy (static) |
| `OrganizerService` | ⚠️ MOCK | 20+ endpoint, semua return Map statis |
| `RefundService` | ⚠️ PARTIAL | Submit+detail REAL; banks/history MOCK |
| `AdminDashboardService` | ✅ REAL | Metrics, recent-events, recent-transactions |
| `AdminUserService` | ✅ REAL | List, detail, status, suspend |

### Services yang BELUM ADA (perlu dibuat)

| Service | Modul | Fungsi |
|---|---|---|
| `OrganizerEventService` | Organizer Events | CRUD event milik EO |
...
### Yang Perlu Di-upgrade ke REAL (MOCK → DB)

- **OrganizerService** (21 endpoint): Semua perlu query ke DB `organizers`, `events`, `refund_requests`
- **RefundService** (3 endpoint): `order-summary`, `download-proof`,`refund-history` perlu query ke DB
- **PaymentService**: Webhook `midtrans-notification` perlu update order status ke PAID
...
# Midtrans (REAL gateway, bukan mock VA):
curl -b cookies.txt -X POST localhost:8082/api/payments/charge -H "Content-Type: application/json" -d '{"orderId":"<order-uuid>","grossAmount":300000,"customerName":"Budi","customerEmail":"budi@mail.com"}'  # return {snapToken, redirectUrl}
curl -b cookies.txt -X POST localhost:8082/api/payments/midtrans-notification -H "Content-Type: application/json" -d '{"order_id":"...","transaction_status":"settlement"}'
