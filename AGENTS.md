...
> **Status 2026-09-21 (rev.14, HEAD `4680644` + uncommitted → committed):** PR #35-43 (dashboard EO, modular folder, ticket fix, split dashboard, payment/refund fix, organizer modular events+tickets, fitur login + WebConfig multipart fix). Perubahan besar vs rev.13: ① **Webhook Midtrans REAL** — SHA-512 signature, PAID → auto `generateTicketsForOrder` + email, cancel/deny/expire → CANCELLED/EXPIRED + restore kuota; ② **Kuota dikurangi saat `checkout/initiate`** + restore via `handleExpiredOrCancelledOrder`; ③ **Organizer events/dashboard REAL** — 6 controller (`controller/organizer/`), 6 service (`service/organizer/`), multipart create/update, publish → `PENDING_APPROVAL`, `POST /api/organizer/events/banner` baru; ④ **Admin events multipart** (`@RequestPart("event")` + `file`) + `PATCH /{id}/approve` + `PATCH /{id}/reject`; ⑤ **Ticket API dirombak** — `GET /user/{email}` dihapus, `my-tickets` auth-based, `scan` wajib ADMIN/ORGANIZER, `issued-detail` anti-IDOR; ⑥ **SecurityConfig** — hanya `GET /api/events/**` publik, `GET /uploads/**` permitAll, alias `/api/v1/payments/**`; ⑦ **AdminEoController** + `company-deed/download`; ⑧ **AdminTicketService** status `CHECKED_IN`/`REVOKED`; ⑨ **FileStorageService** BARU; ⑩ `OrderService` kuota decrement + restore. ⚠️ Build mode aktif, file tambahan sudah di-commit (AdminEventController, OrganizerEventController, FileStorageService). Test: 20 PASS / 2 FAIL pre-existing.
